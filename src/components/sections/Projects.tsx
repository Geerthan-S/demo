        ...
        <div className="relative z-10 max-w-[1360px] mx-auto w-full px-6 lg:px-8">
          ...
          <nav className="premium-projects__filters relative z-20 mt-6 flex flex-wrap justify-center gap-2" aria-label="Featured project categories">
            {categoryPills.map((pill, index) => (
              <Link
                key={pill.label}
                href={pill.href}
                className={`rounded-[10px] border px-5 py-2 text-[11px] font-bold uppercase tracking-wider transition-all duration-300 ${index === 0
          ...
          {featuredHero && (
            <motion.div
              ...
              className="premium-projects__editorial group relative mt-10 hidden w-full flex-col overflow-hidden rounded-[16px] md:flex border border-[#8B3A4A]/10 bg-white shadow-[0_24px_64px_rgba(139,58,74,0.06)] lg:flex-row xl:w-[calc(100%+100px)] xl:-ml-[50px] 2xl:w-[calc(100%+140px)] 2xl:-ml-[70px]"
            >
              {/* Vertical Banner */}
          ...
          >
            {/* Phones show the featured project as a regular card so every
                card in the list reads the same. */}
            {featuredHero && (
              <motion.div
